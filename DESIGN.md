...
## rhINDEX-CL Strategy Architecture & Accounting

`ConcentratedLiquidityStrategy` is a single-pool, hookless Uniswap v4 allocator. It keeps user ownership in RobinVault shares, owns the v4 position NFTs, and routes all returned assets to the immutable vault.

- `StaticRangeRebalancePolicy` supplies target ranges; the strategy alone validates spacing, bounds, and width.
- Official v4 `PoolManager` slot0 reads and `PositionManager.modifyLiquidities` action plans are used through the checked-in remappings.
- Deposits and rebalances use `ExecutionRouter` with oracle-derived minimum outputs.
- V1 maintains a 64-observation, 30-minute internal price ring; hookless pools have no native TWAP.
- TWAP observations are checked against the current block timestamp and must cover the complete configured window;
  expired or short histories cannot authorize withdrawals with active positions.
- CL deviation thresholds are relative sqrt-price BPS, not actual token-price BPS.
- Permit2 uses exact-per-operation approvals that are cleared after settlement.
- `Active`, `HarvestOnly`, `WithdrawOnly`, and `Paused` are separate from `StrategyBase` lifecycle state.
- Withdrawals close positions, convert paired-token proceeds back to INDEX through the constrained router, and enforce loss/TWAP limits.
